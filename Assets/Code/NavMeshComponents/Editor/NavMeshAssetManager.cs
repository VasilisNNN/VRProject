...
namespace UnityEditor.AI
{
    public class NavMeshAssetManager : ScriptableSingleton<NavMeshAssetManager>
    {
        ...
        {
            public NavMeshSurfaceCustom surface;
        ...
        static string GetAndEnsureTargetPath(NavMeshSurfaceCustom surface)
        {
        ...
        static void CreateNavMeshAsset(NavMeshSurfaceCustom surface)
        {
        ...
        NavMeshData GetNavMeshAssetToDelete(NavMeshSurfaceCustom navSurface)
        {
            if (PrefabUtility.IsPartOfPrefabInstance(navSurface) && !PrefabUtility.IsPartOfModelPrefab(navSurface))
            {
                // Don't allow deleting the asset belonging to the prefab parent
                var parentSurface = PrefabUtility.GetCorrespondingObjectFromSource(navSurface) as NavMeshSurfaceCustom;
        ...
        void ClearSurface(NavMeshSurfaceCustom navSurface)
        {
        ...
        public void StartBakingSurfaces(UnityEngine.Object[] surfaces)
        {
            ...
            foreach (NavMeshSurfaceCustom surf in surfaces)
            {
        ...
        static NavMeshData InitializeBakeData(NavMeshSurfaceCustom surface)
        {
        ...
        public bool IsSurfaceBaking(NavMeshSurfaceCustom surface)
        {
            if (surface == null)
                return false;
        ...
        {
            foreach (NavMeshSurfaceCustom s in surfaces)
                ClearSurface(s);
        }

        static void SetNavMeshData(NavMeshSurfaceCustom navSurface, NavMeshData navMeshData)
        {
        ...
        void StoreNavMeshDataIfInPrefab(NavMeshSurfaceCustom surfaceToStore)
        {
            ...
            {
                var basePrefabSurface = PrefabUtility.GetCorrespondingObjectFromSource(surfaceToStore) as NavMeshSurfaceCustom;
        ...
        bool IsCurrentPrefabNavMeshDataStored(NavMeshSurfaceCustom surface)
        {
            if (surface == null)
                return false;
        ...
        void DeleteStoredNavMeshDataAssetsForOwnedSurfaces(GameObject gameObjectInPrefab)
        {
            ...
            var surfaces = gameObjectInPrefab.GetComponentsInChildren<NavMeshSurfaceCustom>(true);
            foreach (var surface in surfaces)
                DeleteStoredPrefabNavMeshDataAsset(surface);
        }

        void DeleteStoredPrefabNavMeshDataAsset(NavMeshSurfaceCustom surface)
        {
        ...
        void ForgetUnsavedNavMeshDataChanges(PrefabStage prefabStage)
        {
            ...
            var allSurfacesInPrefab = prefabStage.prefabContentsRoot.GetComponentsInChildren<NavMeshSurfaceCustom>(true);
            NavMeshSurfaceCustom surfaceInPrefab = null;
            var index = 0;
            do
            {
                ...
                for (var i = m_PrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
                {
                    ...
                    else if (surfaceInPrefab != null && storedPrefabInfo.surface == surfaceInPrefab)
                    {
                        ...
                        var baseSurface = PrefabUtility.GetCorrespondingObjectFromSource(surfaceInPrefab) as NavMeshSurfaceCustom;
