...
namespace Nav3D.API
{
    ...
    public class Nav3DObstacleLoader : MonoBehaviour
    {
        ...
        readonly string BINARY_DATA_IS_NOT_ATTACHED_ERROR = $"Unable to deserialize obstacle. There is no binary data attached into the field {nameof(m_BinaryData)}. Attach a binary.";
        const string NO_SERIALIZED_OBSTACLES = "No serialized obstacles. Bake obstacles in editor mode.";
        const string SERIALIZED_OBSTACLES_HAS_CHANGED = "The last serialized data is not valid because one of the baked obstacles was changed or removed. Re-bake obstacles in editor mode.";
        ...
        readonly string MIN_BUCKET_MISMATCH_ERROR = $"Mismatch between the actual {nameof(Nav3DInitializer)}.{nameof(Nav3DInitializer.MinBucketSize)}: {{0}} " +
        ...
        readonly string INSTANCE_HAS_EXIST_ON_SCENE_ERROR = $"An instance of {nameof(Nav3DObstacleLoader)} is already present in the scene, duplication is not possible.";

        readonly static string ON_INIT_UNSUBSCRIBE_ERROR = $"There is no need to unsubscribe from the {nameof(OnLoadingFinished)} event. " +
        ...
        List<Action> m_OnLoadSubscribers = new List<Action>();
        ...
        public Nav3DObstacle[] GetAllSerializableObstacles()
        {
            return
                ...
                .Where(_Obstacle => _Obstacle.gameObject.activeInHierarchy && _Obstacle.DataSource == Nav3DObstacle.ObstacleDataSource.LOAD_FROM_BINARY)
                .ToArray();
        }
        ...
        public ObstacleSerializingProgress SerializeObstacles(string _FilePath, Action<Nav3DObstacle[]> _OnFinish)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    additionProgress = ObstacleManager.Instance.AddObstacles(preparedObstacleInfos, () =>
                    {
                        if (obstacleSerializingProgress.CancellationToken.IsCancellationRequested)
                            return;

                        obstacleSerializingProgress.SetStatus(ObstacleSerializingStatus.PACKING_BAKED_DATA);

                        Dictionary<ObstacleInfoBase, Obstacle> obstacleDatas = ObstacleManager.Instance.GetObstacleDatas();
                        ...
                        try
                        {
                            UtilsSerialization.SerializeObstacleData(obstacleDataSerializable, _FilePath, obstacleSerializingProgress);
                        }
                        ...
                        obstacleSerializingProgress.SetStatus(ObstacleSerializingStatus.FINISHED);
        ...
        {
            return !m_SerializedObstacles.Any(_Obstacle =>
                    ...
                    _Obstacle.DataSource != Nav3DObstacle.ObstacleDataSource.LOAD_FROM_BINARY //obstacle datasource was changed
                );
        }
        ...
        void LoadObstacles()
        {
            ...
            Nav3DManager.OnNav3DPreInit += () =>
            {
                if (!ValidateInitializerParameters(out float initializerMinBucketSize))
                    throw new Exception(string.Format(MIN_BUCKET_MISMATCH_ERROR, initializerMinBucketSize, m_MinBucketSizeSerialized));
                ...
                Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        foreach (ObstacleInfoBase obstacleInfo in deserializedData.Keys)
                        {
                            for (int i = 0; i < m_SerializedObstacles.Length; i++)
                                obstacleInfo.ReplaceID(i, m_SerializedObstacles[i].InstanceID);
                        }

                        ObstacleManager.Instance.AddBakedObstacles(deserializedData, () =>
                        {
                            m_SerializedObstacles.ForEach(_Obstacle => _Obstacle?.InvokeOnObstacleAdded());
        ...
        bool ValidateInitializerParameters(out float initializerMinBucketSize)
        {
            Nav3DInitializer initializer = FindObjectOfType<Nav3DInitializer>();

            initializerMinBucketSize = initializer.MinBucketSize;
