...
namespace Nav3D.API.Editor
{
    ...
    public class Nav3DObstacleLoaderInspector : UnityEditor.Editor
    {
        ...
        const string DIALOG_RESERIALIZE_MSG = "It looks like you've already selected the obstacle data binary, are you sure you want to bake and serialize the data again?";
        const string DIALOG_RESERIALIZE_YES = "Yes, go ahead";
        const string DIALOG_RESERIALIZE_CANCEL = "Cancel";

        readonly string NAV3D_INITIALIZER_NOT_FOUND = $"The {nameof(Nav3DInitializer)} component is not found on the scene. Without it, Nav3D will not initialize and load obstacles.";
        ...
        const string SERIALIZED_OBSTACLES_HAS_CHANGED_TIP = "The last serialized data is not valid because one of the baked obstacles was changed or removed. Re-bake obstacles.";
        readonly string SERIALIZED_INITIALIZER_PARAMETERS_HAS_CHANGED_TIP = $"Since the last serialization, the size of the minimum bucket cell in Nav3DInitializer has changed. " +
        ...
        void SerializationCompleted()
        {
            ...
            m_BinaryData.objectReferenceValue = (TextAsset)EditorGUILayout.ObjectField(BINARY_LABEL, m_BinaryData.objectReferenceValue, typeof(TextAsset), false);
        ...
        void SerializableObstaclesList()
        {
            ...
            if (m_ObstaclesOnSceneFoldout = EditorGUILayout.BeginFoldoutHeaderGroup(m_ObstaclesOnSceneFoldout, SERIALIZABLE_OBSTACLES_HEADER))
            {
                ...
                scrollSerializableObstaclesListPhase = EditorGUILayout.BeginScrollView(scrollSerializableObstaclesListPhase, GUILayout.MaxHeight(Mathf.Min(m_SerializableObstaclesOnScene.Length * 17, 150)));
        ...
        void NotSerializedObstaclesList(Nav3DObstacle[] _Obstacles)
        {
            ...
            scrollNoSerializedObstaclesListPhase = EditorGUILayout.BeginScrollView(scrollNoSerializedObstaclesListPhase, GUILayout.MaxHeight(Mathf.Min(_Obstacles.Length * 17, 150)));
        ...
        void SerializationInProgress()
        {
            ...
            {
                if (EditorUtility.DisplayCancelableProgressBar(m_SerializingProgress.GetTitle(), m_SerializingProgress.GetInfo(), m_SerializingProgress.GetProgress()))
                {
