...
namespace Nav3D.API
{
    public static class Nav3DManager
    {
        ...
        readonly static string NEED_INIT_ERROR = $"{nameof(Nav3DManager)} needs to be initialized before! " +
            $"All your work with Nav3D entities should begin on the {nameof(Nav3DManager)}.{nameof(OnNav3DInit)} event execution.";

        readonly static string ON_INIT_UNSUBSCRIBE_ERROR = $"There is no need to unsubscribe from the {nameof(OnNav3DInit)} event. " +
            $"All subscriptions will be invoked and unsubscribed after {nameof(Nav3DManager)} is initialized.";

        readonly static string ON_PRE_INIT_UNSUBSCRIBE_ERROR = $"There is no need to unsubscribe from the {nameof(OnNav3DPreInit)} event. " +
            $"All subscriptions will be unsubscribed after {nameof(Nav3DManager)} is initialized.";

        readonly static string NOT_INITIALIZED_WARNING = $"{nameof(Nav3DManager)} is not initialized yet!";
        ...
        {
            public Nav3DManagerNotInitializedException() : base(NEED_INIT_ERROR) { }
        }
        ...
        static List<Action> m_OnInitSubscribers = new List<Action>();
        static List<Action> m_OnInitInternalSubscribers = new List<Action>();
        static List<Action> m_OnPreInitSubscribers = new List<Action>();
        ...
        public static event Action OnNav3DPreInit
        {
            ...
            remove
            {
                Debug.LogError(ON_PRE_INIT_UNSUBSCRIBE_ERROR);
            }
        }
        ...
        public static bool BoundsCrossStaticObstacles(Bounds _Bounds)
        {
            if (!Inited)
                throw new Nav3DManagerNotInitializedException();

            return ObstacleManager.Instance.BoundsCrossStaticObstacles(_Bounds);
        }
        ...
        {
            if (!Inited)
                UnityEngine.Debug.LogWarning(NOT_INITIALIZED_WARNING);
        }
