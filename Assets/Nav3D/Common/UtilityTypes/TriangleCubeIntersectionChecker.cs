...
namespace Nav3D.Common
{
    ...
    public static class TriangleCubeIntersectionChecker
    {
        ...
        {
            Vector3 fromCenterToP1 = (_Triangle.V1 - _Cube.center) / _Cube.size.x;
            Vector3 fromCenterToP2 = (_Triangle.V2 - _Cube.center) / _Cube.size.y;
            Vector3 fromCenterToP3 = (_Triangle.V3 - _Cube.center) / _Cube.size.z;

            Vector3 V12 = fromCenterToP2 - fromCenterToP1;
            Vector3 V23 = fromCenterToP3 - fromCenterToP2;

            return InsideInternal(fromCenterToP1, fromCenterToP2, fromCenterToP3, Vector3.Cross(V12, V23).normalized);
        }
        ...
        static bool InsideInternal(Vector3 _V1, Vector3 _V2, Vector3 _V3, Vector3 _Normal)
        {
        ...
        static long FacePlane(Vector3 _Point)
        {
            long outcode = 0;
        ...
        static long Bevel2D(Vector3 _Point)
        {
            long outcode = 0;
        ...
        static long Bevel3D(Vector3 _Point)
        {
            long outcode = 0;
        ...
        static long CheckLine(Vector3 _Point1, Vector3 _Point2, long _OutcodeDiff)
        {
        ...
        static long CheckPoint(Vector3 _Point1, Vector3 _Point2, float _Alpha, long _Mask)
        {
            Vector3 planePoint;
        ...
        static long PointTriangleIntersection(Vector3 _Point, Vector3 _V1, Vector3 _V2, Vector3 _V3)
        {
            long sign12, sign23, sign31;
            Vector3 vect1h, vect2h, vect3h;
            Vector3 cross12_1p, cross23_2p, cross31_3p;

            /* First, a quick bounding-box test:                               */
            /* If P is outside triangle bbox, there cannot be an intersection. */

            if (_Point.x > Mathf.Max(_V1.x, _V2.x, _V3.x)) return OUTSIDE;
            if (_Point.y > Mathf.Max(_V1.y, _V2.y, _V3.y)) return OUTSIDE;
            if (_Point.z > Mathf.Max(_V1.z, _V2.z, _V3.z)) return OUTSIDE;
            if (_Point.x < Mathf.Min(_V1.x, _V2.x, _V3.x)) return OUTSIDE;
            if (_Point.y < Mathf.Min(_V1.y, _V2.y, _V3.y)) return OUTSIDE;
            if (_Point.z < Mathf.Min(_V1.z, _V2.z, _V3.z)) return OUTSIDE;
            ...
            vect1h = _Point - _V1;
            cross12_1p = Vector3.Cross(_V2 - _V1, vect1h);
            sign12 = SIGN3(cross12_1p);      /* Extract X,Y,Z signs as 0..7 or 0...63 integer */


            vect2h = _Point - _V2;
            cross23_2p = Vector3.Cross(_V3 - _V2, vect2h);
            sign23 = SIGN3(cross23_2p);


            vect3h = _Point - _V3;
            cross31_3p = Vector3.Cross(_V1 - _V3, vect3h);
        ...
        static float LERP(float A, float B, float C) => B + A * (C - B);
        static long SIGN3(Vector3 A) =>
