...
namespace Nav3D.Common
{
    class CurvesSpatialHashMap<T> where T : ICurve
    {
        ...
        readonly string INVALID_BUCKET_SIZE = "Invalid bucket size: {0}. The bucket size must be greater than zero.";
        ...
        public void Register(T _Curve)
        {
            ...
            foreach (Ray segment in _Curve.Segments)
            {
                buckets.AddRange(LineVoxelizer.GetVoxels(segment, m_BucketSize));
            }

            m_Curves.TryAdd(_Curve, buckets);
            buckets.ForEach(_Bucket => m_Buckets.GetOrAdd(_Bucket, new ConcurrentHashSet<T>()).Add(_Curve));
        }
        ...
        public void Draw()
        {
            ...
            {
                UtilsMath.GetBucket(bucket, m_BucketSize).Draw();
            }
        }
